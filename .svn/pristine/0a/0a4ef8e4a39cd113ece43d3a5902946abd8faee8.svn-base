...
export default class FactoryEditView extends Component {
    constructor(props) {
        super(props);
    ...
    onEditFactory = async () => {
        const { title, address, lat, lon, factory } = this.state;
        ...
        if (this.validateInputs()) {
            ...
            const response = await HttpRequest(body, "/Factories/UpdateFactory");
    ...
    render() {
        ...
        return (
            <div>
                <Header />
                <Menu pageName={PageConstant.FACTORY_EDIT_PAGE_URL} />
                <div className="content-wrapper pt-3">
                    <PageContent>
                        ...
                        <div class="card card-info">
                            <div className="card-body">
                                <FormColumn title="İsim" onChange={this.changeInput} value={title} name={"title"} />
                                <FormColumn title="Açık Adres" onChange={this.changeInput} value={address} name={"address"} />
                                <div className="form-group row">
                                    <div className="col-sm-12 mt-3">
                                        <button disabled={isLoading} onClick={this.onEditFactory} class="btn btn-block btn-outline-success">
                                            {isLoading ? (
